...
if __name__ == '__main__':
    data = "Netflix"
    top_k = 10
    mlp_layers = "[50,32,16,8]"
    mlp_regulation_rate = 0.0001
    mlp_epoch = 1  # the training epoch of PMLP
    alpha = 0.8 # the balance factor

    with open("../SVD++/data/SVD++_emb_file_" + data, 'rb') as f:
    ...
    # with open('data/QUAVector_' + data, 'wb') as f:
    ...
    # Initialize CLA
    with open("data/QUAVector_Netflix", 'rb') as f:
        emb_file = pickle.load(f)
    ...
    with open("data/user_ranklist_" + data, 'wb') as f:
        pickle.dump(ranks, f)
    with open("data/ground_truth_" + data, 'wb') as f:
    ...
    print("precision = %.4f, recall = %.4f, NDCG = %.4f, HR = %.4f " % (precision, recall, NDCG, HR))
